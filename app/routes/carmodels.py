...
from app.utils.errorlog import log_error_to_db  # ✅ Import logger

router = APIRouter(prefix="/carmodels", tags=["Carmodels"])

@router.get("/search/", response_model=list[CarModelResponse])
async def search_cars(query: str, request: Request, db: AsyncSession = Depends(get_async_db)):
    ...
    try:
        ...
        # ✅ Step 2: Try company similarity search using Trigram
        sql = text("""
        ...
        # ✅ Step 3: Fallback to fuzzy search by model name
        ...
        # ✅ Log the error into DB and return generic error
